    ...
    public class ProductDAO
    {
        ...
        public Product GetProduct(int productId, IEnumerable<Product> searchList = null)
        {
            ...
            try
            {
                ...
                {
                    var context = new FstoreContext();
                    var p = context.Products.Where(pro => pro.ProductId == productId)
        ...
        public Product GetProduct(string productName, IEnumerable<Product> searchList = null)
        {
            ...
            try
            {
                ...
                {
                    var context = new FstoreContext();
                    var p = context.Products.Where(pro => pro.ProductName.Equals(productName))
                        .Include(pro => pro.Category);
                    if (p != null && p.Any())
                    {
                        product = p.First();
            ...
            try
            {
                if (searchList == null)
                {
                    ...
                    searchResult = context.Products
                                        ...
                                        .Include(pro => pro.Category).ToList();
                    var aaa = context.Orders
                         .Where(pro => pro.RequiredDate == DateTime.Now)
                        .Include(pro => pro.OrderDetails).ToList();
